...
describe UserSpaceParticipation do
  ...
  describe "finders - " do
    ...
    it "should return all request for helps by space_id, user_id and period" do
      subject.helps(@users_id[0]).should == @helps.length
    end

    it "should return all request for activies by user and period" do
      subject.activities(@users_id[1]).should == @activities.length
    end

    it "should return all request for answer from helps by user and period" do
      subject.answered_helps(@users_id[2]).should == @answered_helps.length
    end

    it "should return all request for answers from acitivies by user and period" do
      subject.answered_activities(@users_id[0]).should == @answered_activities.length
    end

    it "should return a average grade by user, space and period" do
